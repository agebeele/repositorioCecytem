...
public class formulario_turno extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        numeroControl = findViewById(R.id.matricula_cambioTurno);
        grupo = findViewById(R.id.grupo__cambioTurno);
        paterno = findViewById(R.id.apellidoPaterno_cambioTurno);
        materno = findViewById(R.id.apellidoMaterno_cambioTurno);
        nombre = findViewById(R.id.nombres_cambioTurno);
        promedio = findViewById(R.id.promedio_cambioTurno);
        carreraActual = findViewById(R.id.spinnerTurno_carreraActual);
        carreraCambio = findViewById(R.id.spinnerTurno_carreraCambio);
        motivos = findViewById(R.id.motivos_cambioTurno);
    }
    public void enviarDatos_cambioTurno(View view) {
        if (numeroControl.getText().toString().isEmpty()
            ...
            Toast.makeText(formulario_turno.this, "Solicitud enviada.", Toast.LENGTH_SHORT).show();
        }
    }
    class MiAsyncTask extends AsyncTask<String, String, Void> {
        ...
        protected Void doInBackground(String... parameter) {
            ...
            switch (parameter[0]) {
                case "solicitar":
                    msj = obj.solicitarCambioTurno(parameter[1], parameter[2], parameter[3], parameter[4], parameter[5], parameter[6], parameter[7],parameter[8], parameter[9],parameter[10], parameter[11]);
                    publishProgress(msj);
                    break;
            }
            return null;
        ...
        protected void onProgressUpdate(String... progress) {
            ...
            try {
                ...
                for (int i = 0; i < jArray.length(); i++) {
                    ...
                    String carreraActualValue = json_data.getString("turnoActual");
                    String carreraCambioValue = json_data.getString("turnoCambio");
